...
timestamp = "20240902001446"


@dataclass
class CFG:
    model_name = "vit_small_patch16_224.augreg_in21k_ft_in1k"  # 22.1M
    ...
    ratio_int = 1
    ratio_int_val = 10  # 10
    ratio_upsampling = 2  # 2
    remap = "case1"  # None, case1, case2
...
print("-" * 20)
print("処理の妥当性を確認（target=0におけるlesion_idが消えていることを確認）")
print(df_train[~df_train["isic_id"].isin(exclude_isic_ids)].groupby(["fold", "target"])["lesion_id"].count())

# %%
# Re-label
# case1
if cfg.remap == "case1":
    filter = df_train["iddx_1"] == "Indeterminate"
    df_train.loc[filter, "target"] = 1
    filter = df_train["iddx_2"].notna()
    df_train.loc[filter, "target"] = 1

# case2
if cfg.remap == "case2":
    filter = df_train["lesion_id"].notna()
    df_train.loc[filter, "target"] = 1
...
def cross_validation_train(
    ...
    for fold in train_folds:
        ...
        def get_optimizer_params(model, lr_ini, lr_decay_rate=1.0, weight_decay=0.0):
            ...
            elif "efficientnetv2" in model.model_name:
                layers = [model.model.conv_stem] + [model.model.conv_head] + list(model.model.blocks)
            elif "eva02_tiny_patch14_224.mim_in22k" in model.model_name:
                layers = [model.model.patch_embed] + list(model.model.blocks)
            elif "eva02_small_patch14_224.mim_in22k" in model.model_name:
                layers = [model.model.patch_embed] + list(model.model.blocks)
            elif "vit_small" in model.model_name:
                layers = [model.model.patch_embed] + list(model.model.blocks)
...
# 例：たとえば、cfg.train_folds=[0]の場合、oof_predictionsのnp.arrrayはfold=0のスコアだけ格納され、ほかの要素は0になる
...
# %%
# OOF pAUC calc
# oof_df = pd.read_csv(Path("output_20240820163852") / "oof_predictions.csv")
# oof_df = pd.read_csv(Path("output_20240820151903") / "oof_predictions.csv")
official_scores = []
